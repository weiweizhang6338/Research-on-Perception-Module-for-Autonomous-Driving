...
由于图像信息位置的巨大差异，为卷积操作选择合适的卷积核大小就比较困难。信息分布更全局性的图像偏好较大的卷积核，信息分布比较局部的图像偏好较小的卷积核。作者考虑能否在同一层级上运行具备多个尺寸的滤波器，以此替代人工确定该层卷积层中滤波器的类型或者是否创建卷积层和池化层，让网络自行学习该层的网络结构，由此提出了Inception module结构，其分别使用1x1, 3x3, 5x5的卷积核分别对前一层的feature map进行卷积，以覆盖图像中不同大小的物体，通过padding操作可使输出的feature map的shape相同，然后在channel方向联合这些feature map以作为当前层的输出。具体的Inception结构如下图所示：
